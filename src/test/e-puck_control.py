...
# ################### # CONTROL THREAD ## ################### One thread is created for each robot. The thread is
# responsible of: initiating and maintaining the communication; interpreting the data received and controlling the
# robot based on these data. Some global variables are used. In the thread these variables are accessed based on the
    ...
    while True:
        ...
        while (expected_recv_packets > 0):
            ...
            if header == bytearray([1]):  # Get a QQVGA image
                ...
                # Here goes your controller. This is a simple example that turn on the body led when there is
                # something in front of any of the proximity sensors.
                if (proximity[client_index][0] >= SENS_THRESHOLD) or (proximity[client_index][1] >= SENS_THRESHOLD) or (
                        proximity[client_index][2] >= SENS_THRESHOLD) or (
                        proximity[client_index][3] >= SENS_THRESHOLD) or (
                        proximity[client_index][4] >= SENS_THRESHOLD) or (
                        proximity[client_index][5] >= SENS_THRESHOLD) or (
                        proximity[client_index][6] >= SENS_THRESHOLD) or (proximity[client_index][7] >= SENS_THRESHOLD):
...
for i in range(10):
    ...
    rgb565 = flatNumpyArray.reshape(160, 120, 2)
    rgb24 = np.empty([160, 120, 3], dtype=int)
    for i in range(160):
        for j in range(120):
            # rgb565[i,j,1] = 248
            # print(rgb565[i, j, 1])
            # print(rgb565[i, j, 0])
            img_byte = rgb565[i, j, 1] * 256 + rgb565[i, j, 0]
            # print(img_byte)
            rgb24[i, j, 2] = img_byte & 0xF800
            rgb24[i, j, 1] = img_byte & 0x07E0
            rgb24[i, j, 0] = img_byte & 0x001F
    cv2.imwrite('img{}.png'.format(i), rgb24)
    print("#\tID\tIP\tSensor\tActuator\t\tN.packets")
    for x in range(NUM_ROBOTS):
        print(str(x) + "\t" + robot_id[x] + "\t" + addr[x] + "\t" + str(proximity[x][0]) + "\t" + str(
            led_state[x]) + "\t\t" + str(num_packets[x]))
        num_packets[x] = 0
    print("\r\n")
